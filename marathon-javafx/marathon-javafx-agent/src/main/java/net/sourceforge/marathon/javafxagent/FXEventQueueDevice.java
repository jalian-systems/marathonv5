    ...
    @Override public void buttonUp(EventTarget node, Buttons button, double xoffset, double yoffset) {
        ...
        dispatchEvent(
                new MouseEvent(MouseEvent.MOUSE_RELEASED, 0, 0, 0, 0, mb, 1, deviceState.shiftPressed, deviceState.ctrlPressed,
                        deviceState.altPressed, deviceState.metaPressed, true, false, false, false, false, false, null),
                node);
    ...
    @Override public void moveto(EventTarget node, double xoffset, double yoffset) {
        ...
        if (node != deviceState.getNode()) {
            if (deviceState.getNode() != null)
                dispatchEvent(new MouseEvent(MouseEvent.MOUSE_PRESSED, xoffset, yoffset, 0, 0, buttons, 0, deviceState.shiftPressed,
            ...
            dispatchEvent(new MouseEvent(MouseEvent.MOUSE_ENTERED, xoffset, yoffset, 0, 0, buttons, 0, deviceState.shiftPressed,
                    deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, buttons == MouseButton.PRIMARY,
                    buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY, false, false, false, null), node);
        }
        ...
        MouseEvent mouseEvent = new MouseEvent(id, xoffset, yoffset, 0, 0, buttons, 0, deviceState.shiftPressed,
                deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, modifierEx == MouseButton.PRIMARY,
                modifierEx == MouseButton.MIDDLE, modifierEx == MouseButton.SECONDARY, false, false, false, null);
        dispatchEvent(mouseEvent, source);
        EventQueueWait.empty();
        ...
        if (node != deviceState.getNode()) {
            if (deviceState.getNode() != null)
                dispatchEvent(new MouseEvent(MouseEvent.MOUSE_EXITED, x, y, scaleX, scaleY, buttons, clickCount,
                        ...
                        false, false, false, popupTrigger, false, null), node);
            dispatchEvent(new MouseEvent(MouseEvent.MOUSE_ENTERED, x, y, scaleX, scaleY, buttons, clickCount,
                    deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, false,
                    false, false, false, popupTrigger, false, null), node);
        }
        for (int n = 1; n <= clickCount; n++) {
            dispatchEvent(new MouseEvent(MouseEvent.MOUSE_PRESSED, x, y, scaleX, scaleY, buttons, n, deviceState.shiftPressed,
                    deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, buttons == MouseButton.PRIMARY,
                    buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY, false, popupTrigger, false, null), node);
            dispatchEvent(new MouseEvent(MouseEvent.MOUSE_RELEASED, x, y, scaleX, scaleY, buttons, n, deviceState.shiftPressed,
                    deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, buttons == MouseButton.PRIMARY,
                    buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY, false, popupTrigger, false, null), node);
            dispatchEvent(new MouseEvent(MouseEvent.MOUSE_CLICKED, x, y, scaleX, scaleY, buttons, n, deviceState.shiftPressed,
                    deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed, buttons == MouseButton.PRIMARY,
                    buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY, false, popupTrigger, false, null), node);
        }
    }
