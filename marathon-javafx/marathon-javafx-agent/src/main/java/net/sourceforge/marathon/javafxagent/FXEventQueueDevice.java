...
public class FXEventQueueDevice implements IDevice {
	...
	public static class DeviceState {
		...
		public String toString() {
			return "DeviceState [shiftPressed=" + shiftPressed + ", ctrlPressed=" + ctrlPressed + ", altPressed="
					+ altPressed + ", metaPressed=" + metaPressed + ", button1Pressed=" + button1Pressed
					+ ", button2Pressed=" + button2Pressed + ", button3Pressed=" + button3Pressed + ", y=" + y + ", x="
					+ x + "]";
		}
	...
	private void dispatchKeyEvent(final Node node, JavaAgentKeys keyToPress, EventType<KeyEvent> eventType, char c) {
		if (keyToPress == null) {
			...
			if (modifiersEx == 0) {
				...
				dispatchEvent(
						new KeyEvent(eventType, char1 + "", char1 + "", keyCode, deviceState.isShiftPressed(),
								deviceState.isCtrlPressed(), deviceState.isAltPressed(), deviceState.isMetaPressed()),
						node);
			} else {
				dispatchEvent(
						new KeyEvent(eventType, char1 + "", char1 + "", keyCode, deviceState.isShiftPressed(),
		...
		dispatchEvent(new KeyEvent(eventType, KeyCode.UNDEFINED.impl_getChar(), KeyCode.UNDEFINED.impl_getChar(),
				keysMap.getCode(), deviceState.isShiftPressed(), deviceState.isCtrlPressed(),
				deviceState.isAltPressed(), deviceState.isMetaPressed()), node);
		EventQueueWait.empty();
	}
	...
	public void buttonDown(Node node, Buttons button, double xoffset, double yoffset) {
		...
		dispatchEvent(createMouseEvent(MouseEvent.MOUSE_PRESSED, xoffset, yoffset, 0, 0, mb, 1,
				deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed,
				true, false, false, false, false, false, node));
	...
	public void buttonUp(Node node, Buttons button, double xoffset, double yoffset) {
		MouseButton mb = button.getMouseButton();
		dispatchEvent(createMouseEvent(MouseEvent.MOUSE_RELEASED, xoffset, yoffset, 0, 0, mb, 1,
				deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed,
				true, false, false, false, false, false, node));
	...
	public void moveto(Node node, double xoffset, double yoffset) {
		...
		if (node != deviceState.getNode()) {
			if (deviceState.getNode() != null)
				dispatchEvent(createMouseEvent(MouseEvent.MOUSE_PRESSED, xoffset, yoffset, 0, 0, buttons, 0,
						deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed,
						deviceState.metaPressed, buttons == MouseButton.PRIMARY, buttons == MouseButton.MIDDLE,
						buttons == MouseButton.SECONDARY, false, false, false, node));
			dispatchEvent(createMouseEvent(MouseEvent.MOUSE_ENTERED, xoffset, yoffset, 0, 0, buttons, 0,
					deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed,
					buttons == MouseButton.PRIMARY, buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY,
		...
		dispatchEvent(createMouseEvent(id, xoffset, yoffset, 0, 0, buttons, 0, deviceState.shiftPressed,
				deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed,
				modifierEx == MouseButton.PRIMARY, modifierEx == MouseButton.MIDDLE,
				modifierEx == MouseButton.SECONDARY, false, false, false, source));
		EventQueueWait.empty();
	...
	@Override
	public void click(Node node, Buttons button, int clickCount, double xoffset, double yoffset) {
	...
	private void dispatchMouseEvent(Node node, boolean popupTrigger, int clickCount, MouseButton buttons, double x,
			double y) {
		Point2D screenXY = node.localToScreen(new Point2D(x, y));
		if (node != deviceState.getNode()) {
			if (deviceState.getNode() != null)
				dispatchEvent(createMouseEvent(MouseEvent.MOUSE_EXITED, x, y, screenXY.getX(), screenXY.getY(), buttons,
						clickCount, deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed,
						deviceState.metaPressed, false, false, false, false, popupTrigger, false, node));
			dispatchEvent(createMouseEvent(MouseEvent.MOUSE_ENTERED, x, y, screenXY.getX(), screenXY.getY(), buttons,
					clickCount, deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed,
					deviceState.metaPressed, false, false, false, false, popupTrigger, false, node));
		}
		for (int n = 1; n <= clickCount; n++) {
			dispatchEvent(createMouseEvent(MouseEvent.MOUSE_PRESSED, x, y, screenXY.getX(), screenXY.getY(), buttons, n,
					deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed, deviceState.metaPressed,
					buttons == MouseButton.PRIMARY, buttons == MouseButton.MIDDLE, buttons == MouseButton.SECONDARY,
					false, popupTrigger, false, node));
			dispatchEvent(createMouseEvent(MouseEvent.MOUSE_RELEASED, x, y, screenXY.getX(), screenXY.getY(), buttons,
					n, deviceState.shiftPressed, deviceState.ctrlPressed, deviceState.altPressed,
					deviceState.metaPressed, buttons == MouseButton.PRIMARY, buttons == MouseButton.MIDDLE,
					buttons == MouseButton.SECONDARY, false, popupTrigger, false, node));
	...
	private MouseEvent createMouseEvent(EventType<? extends MouseEvent> eventType, double x, double y, double screenX,
		...
		return new MouseEvent(source, target, eventType, x, y, screenX, screenY, button, clickCount, shiftDown,
				controlDown, altDown, metaDown, primaryButtonDown, middleButtonDown, secondaryButtonDown, synthesized,
				popupTrigger, stillSincePress, pickResult);
	}
