...
public class JavaServer extends NanoHTTPD {
	...
	static {
		...
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/timeouts"));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/timeouts/async_script"));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/timeouts/implicit_wait",
				getMethod("setImplicitTimeout")));
		...
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/window/:windowHandle/size",
				getMethod("setWindowSize", true)));
		routes.add(
				new RouteMap(Method.GET, "/session/:sessionId/window/:windowHandle/size", getMethod("getWindowSize")));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/window/:windowHandle/position",
				getMethod("setWindowPosition", true)));
		routes.add(new RouteMap(Method.GET, "/session/:sessionId/window/:windowHandle/position",
				getMethod("getWindowPosition")));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/window/:windowHandle/maximize",
				getMethod("maximizeWindow", true)));
		...
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/element/:id/element",
				getMethod("findElementOfElement")));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/element/:id/elements",
				getMethod("findElementsOfElement")));
		routes.add(new RouteMap(Method.POST, "/session/:sessionId/element/:id/click", getMethod("clickElement", true)));
		routes.add(
				new RouteMap(Method.POST, "/session/:sessionId/element/:id/submit", getMethod("submitElement", true)));
		routes.add(new RouteMap(Method.GET, "/session/:sessionId/element/:id/text", getMethod("getElementText")));
		routes.add(
				new RouteMap(Method.POST, "/session/:sessionId/element/:id/value", getMethod("sendKeysElement", true)));
		...
		routes.add(new RouteMap(Method.GET, "/session/:sessionId/element/:id/attribute/:name",
				getMethod("getElementAttribute")));
		routes.add(
				new RouteMap(Method.GET, "/session/:sessionId/element/:id/equals/:other", getMethod("elementEquals")));
		routes.add(new RouteMap(Method.GET, "/session/:sessionId/element/:id/displayed", getMethod("isDisplayed")));
		routes.add(
				new RouteMap(Method.GET, "/session/:sessionId/element/:id/location", getMethod("getElementLocation")));
	...
	public Response serve_internal(String uri, Method method, JSONObject jsonQuery) {
		try {
			...
			if (route == null)
				return nanoHTTPDnewFixedLengthResponse(Status.NOT_FOUND, MIME_PLAINTEXT,
						"Not Implemented: (route is null)");
			if (route.getProc() == null)
				return nanoHTTPDnewFixedLengthResponse(Status.INTERNAL_ERROR, MIME_PLAINTEXT,
						"Not Implemented: route = " + route);
		} catch (Throwable e) {
	...
	private Response handleRoute(Route route, JSONObject query) {
		...
		try {
			...
			return nanoHTTPDnewFixedLengthResponse(Status.BAD_REQUEST, MIME_PLAINTEXT, e.getMessage());
		} catch (UnsupportedCommandException e) {
			return nanoHTTPDnewFixedLengthResponse(Status.METHOD_NOT_ALLOWED, MIME_PLAINTEXT, e.getMessage());
			...
			r.put("value", new JSONObject().put("message", e.getClass().getName() + ":" + e.getMessage())
	...
	private String hasCapabilities(JSONObject required) {
		...
		while (keys.hasNext()) {
			...
			if (!lvalue.equals(rvalue)) {
				...
				return "Java Driver does not support `" + key + "`"
						+ (rvalue instanceof Boolean ? "" : " for value " + rvalue);
			}
		}
		return null;
	}
	...
	public JSONObject getElementLocation(JSONObject query, JSONObject uriParams, Session session,
			IJavaFXElement element) {
	...
	public JSONObject findElementOfElement(JSONObject query, JSONObject uriParams, Session session,
			IJavaFXElement element) {
		checkRequiredArguments(query, "using", "value");
	...
	public JSONArray findElementsOfElement(JSONObject query, JSONObject uriParams, Session session,
			IJavaFXElement element) {
		checkRequiredArguments(query, "using", "value");
		JSONArray r = new JSONArray();
	...
	public void moveto(JSONObject query, JSONObject uriParams, Session session) {
		...
		if (hasOffset != query.has("yoffset"))
			throw new MissingCommandParametersException("Missing x-offset or y-offset. Provide both x and y offsets.",
					null);
	...
	public Object execute(JSONObject query, JSONObject uriParams, Session session) {
		...
		JSONScriptRunner scriptRunner = new JSONScriptRunner(query.getString("script"), query.getJSONArray("args"),
				session, ExecuteMode.SYNC);
