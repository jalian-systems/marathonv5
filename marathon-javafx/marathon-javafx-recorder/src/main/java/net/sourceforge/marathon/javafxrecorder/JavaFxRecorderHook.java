...
public class JavaFxRecorderHook implements EventHandler<Event> {
	...
	private void handle_internal(Event event) {
		...
		if (recordWindowState != null && recorder.isRawRecording()) {
			recorder.recordWindowState(new WindowTitle(recordWindowState).getTitle(),
					recordWindowState.xProperty().intValue(), recordWindowState.yProperty().intValue(),
					recordWindowState.widthProperty().intValue(), recordWindowState.heightProperty().intValue());
		...
		if (recorder.isRawRecording()) {
			new RFXUnknownComponent((Node) event.getTarget(), objectMapConfiguration, null, recorder)
