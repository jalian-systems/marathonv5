...
describe "Command line" do
  before(:each) do
    @profile = Marathon::JavaDriver::Profile.new :command_line
    @profile.set_command(get_command)
  end
  ...
  if @profile.windows? then 
      f = File.new(File.join(File.dirname(__FILE__), '../../marathon-test-helpers/swingset3/swingset3.bat'), 'r')
      command = File.expand_path(f)
    else 
      f = File.new(File.join(File.dirname(__FILE__), '../../marathon-test-helpers/swingset3/swingset3.sh'), 'r')
      command = File.expand_path(f)
    end
  end
