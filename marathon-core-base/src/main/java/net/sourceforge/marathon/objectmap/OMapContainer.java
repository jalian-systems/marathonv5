...
public class OMapContainer implements IObjectMapTreeItem {
    ...
    private List<String> containerTitles = new ArrayList<>();
    private List<OMapProperty> containerGeneralProperties = new ArrayList<>();
    private List<OMapComponent> components = new ArrayList<>();
    private List<OMapRecognitionProperty> containerRecognitionProperties = new ArrayList<>();
    private Map<String, OMapComponent> nameComponentMap = new HashMap<>();
    ...
    public OMapContainer(ObjectMapItem parent, String title) {
        ...
        OMapProperty op = new OMapProperty();
        op.setName("component.class.name");
        op.setValue("Unknown");
        containerGeneralProperties.add(op);
        if (this.parent != null) {
            fileName = createFileName(parent.getOmapFolder(), title);
            loaded = true;
        }
    }
    ...
    private String createFileName(File omapFolder, String title) {
        try {
            if (title.length() < 3) {
                title = title + "___";
            ...
            return File.createTempFile(sanitize(title) + "_", ".yaml", omapFolder).getName();
        } catch (IOException e) {
 ...
 // @formatter:off
   ...
   // @formatter:on
    ...
    public void load(File omapFolder) throws FileNotFoundException {
        ...
        components = (List<OMapComponent>) loadYaml(new File(omapFolder, fileName));
        if (components == null) {
    ...
    public void removeContainerRecognitionProperty(OMapRecognitionProperty property) {
        containerRecognitionProperties.remove(property);
    }
    ...
    public void save(File omapFolder) throws IOException {
        logger.info("Saving object map container " + containerRecognitionProperties);
        File file = new File(omapFolder, fileName);
    ...
    public void deleteFile(File omapFolder) {
        File file = new File(omapFolder, fileName);
    ...
    public List<OMapComponent> getComponents() {
        return components;
    }

    public ObjectMapItem getParent() {
        return parent;
    }
