...
public class ObjectMapItem implements IObjectMapTreeItem {
    ...
    public ObjectMapItem(File oMapFile) {
        omapFolder = new File(oMapFile.getParentFile(), Constants.DIR_OMAP);
        if (!omapFolder.exists()) {
            if (!omapFolder.mkdirs()) {
    ...
    protected void loadContainer(OMapContainer container) throws FileNotFoundException {
        container.load(omapFolder);
    }
    ...
    public File getOmapFolder() {
        return omapFolder;
    }

    public List<OMapContainer> getContainers() {
        return data;
    }
}
