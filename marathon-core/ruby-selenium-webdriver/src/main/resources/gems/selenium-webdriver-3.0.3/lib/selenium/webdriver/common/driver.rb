      ...
      class << self
        ...
        def for(browser, opts = {})
          ...
          bridge = case browser
                   ...
                   when :remote
                     browser = :firefox
                     browser = opts[:desired_capabilities][:browserName].to_sym if opts[:desired_capabilities] and opts[:desired_capabilities][:browserName]
                     marionette = false
                     marionette = opts[:desired_capabilities][:marionette] if opts[:desired_capabilities] and opts[:desired_capabilities][:marionette]
                     puts '******************* Browser = ' + browser.to_s + "(marionette=" + marionette.to_s + ")"
                     if browser == :edge
                        puts '******************* Creating Edge Bridge'
                        Edge::Bridge.new(opts)
                     elsif marionette
                        puts '******************* Creating W3C Bridge'
                        Remote::W3CBridge.new(opts)
                     else
                        puts '******************* Creating Normal Bridge'
