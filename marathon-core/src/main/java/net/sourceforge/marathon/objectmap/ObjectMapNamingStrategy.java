...
public class ObjectMapNamingStrategy implements INamingStrategy {
	...
	public void init() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Error in creating naming strategy:" + e.getMessage(),
					"Error in NamingStrategy", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
			System.exit(1);
		}
	}
	...
	private String toCSS(OMapComponent omapComponent, boolean visibility) {
		...
		if (tagNameProperty != null) {
			if (tagNameProperty.getMethod().equals("equals"))
				...
				r = "[" + tagNameProperty.getName() + op(tagNameProperty.getMethod()) + "'" + tagNameProperty.getValue()
						+ "']" + r;
		}
		return r;
	}
	...
	private OMapComponent findClosestMatch(JSONObject component, List<OMapComponent> omapComponents,
			StringBuilder msg) {
		return null;
	}
