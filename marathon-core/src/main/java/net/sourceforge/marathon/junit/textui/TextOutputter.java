...
public class TextOutputter implements IOutputter {
    ...
    @Override public void output(Writer writer, Test test, Map<Test, MarathonTestResult> testOutputMap) throws IOException {
    ...
    private void printHeader(Writer writer, TestSuite testSuite, Map<Test, MarathonTestResult> testOutputMap) throws IOException {
        ...
        int totalFailures = 0;
        int totalErrors = 0;
        for (MarathonTestResult marathonTestResult : values) {
            MarathonTestResult result = marathonTestResult;
    ...
    private void printDetails(Writer writer, Test test, Map<Test, MarathonTestResult> testOutputMap) throws IOException {
        if (test instanceof TestSuite) {
            ...
            while (testsEnum.hasMoreElements()) {
                printDetails(writer, testsEnum.nextElement(), testOutputMap);
            }
        } else {
            if (test != null) {
