...
public class RubyScriptModel implements IScriptModel {
	...
	public void createDefaultFixture(JDialog parent, Properties props, File fixtureDir, List<String> keys) {
		...
		if (fixtureFile.exists()) {
			int option = JOptionPane.showConfirmDialog(parent,
					"File " + fixtureFile + " exists\nDo you want to overwrite", "File Exists",
		...
		try {
			...
			props.setProperty(Constants.FIXTURE_DESCRIPTION,
					props.getProperty(Constants.FIXTURE_DESCRIPTION, "Default Fixture"));
	...
	public String getFunctionCallForInsertDialog(Function function, String[] arguments) {
		...
		if (arguments.length != 0) {
			buffer.append(
					encodeArg(arguments[arguments.length - 1], function.getArguments().get(arguments.length - 1)));
		}
	...
	public String getModuleHeader(String moduleFunction, String description) {
		String prefix = "=begin" + EOL + description + EOL + "=end" + EOL + EOL + "def " + moduleFunction + "()" + EOL
				+ EOL + "    ";
	...
	@Override
	public IScript createScript(Writer out, Writer err, String scriptText, String filePath, boolean isRecording,
	...
	@Override
	public String getScriptCodeForGenericAction(String method, String name, Object... params) {
