...
public class RSTAEditor extends RSyntaxTextArea implements IEditor, DocumentListener, CaretListener, PreferenceChangeListener {
    ...
    public void setFontPreference() {
        ...
        if (font != null) {
            for (int i = 0; i < syntaxScheme.getStyles().length; i++) {
                if (syntaxScheme.getStyles()[i] != null) {
                    syntaxScheme.getStyles()[i].font = font;
                }
            }
    ...
    private Font setTextAreaFont() {
        ...
        String fontName = p.get("editor.font", "Monospace");
        String fontSize = p.get("editor.fontsize", "13");
    ...
    public int find(String searchText, boolean bForward, boolean bAllLines, boolean bCaseSensitive, boolean bWrapSearch,
        ...
        this.regex = bRegex;
        SearchContext context = createContext(searchText, bCaseSensitive, bWholeWord, bRegex);
        SearchResult b = SearchEngine.find(this, context);
        if (!b.wasFound())
            beep();
        if (!b.wasFound() && bWrapSearch) {
            if (bForward)
                setCaretPosition(0);
            else
                setCaretPosition(getDocument().getLength() - 1);
            b = SearchEngine.find(this, context);
            if (b.wasFound())
                return FIND_WRAPPED;
        }
        return b.wasFound() ? FIND_SUCCESS : FIND_FAILED;
    }

    protected SearchContext createContext(String searchText, boolean bCaseSensitive, boolean bWholeWord, boolean bRegex) {
        SearchContext context = new SearchContext(searchText, bCaseSensitive);
        context.setRegularExpression(bRegex);
        context.setWholeWord(bWholeWord);
        return context;
    }
